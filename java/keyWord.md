...
有助于提升性能, 修饰基本数据类型和String类型可以替换为对应的值, 修饰方法因为子类不能重写这些方法因此jvm可以对这些方法进行优化提升执行效率
...
如果读取,直接读取私有内存中的变量值,如果修改,先修改私有内存中的变量值.然后同步到jvm内存中,
...
+ 对象锁:如果直接修饰方法那么锁定的this(当前对象),如果是修饰代码块的形式那么括号里面传入的是什么,锁定的就是什么,可以是this也可以是指定的对象实例
